...
export default function NotificationItem({ item, onMarkRead, onDelete }: Props) {
  ...
  return (
    <NotificationStatusBorder status={item.status} className={`my-0.5 ${!item.readStatus ? "bg-gray-50/70 dark:bg-gray-800" : ""}`}>
      <div className="flex items-start gap-2 ml-1 py-2 pr-2" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-gray-400 dark:text-white/40 mt-0.5">{formatDate(parseInt(item.createdAt))}</p>
        </div>
        <div className="relative shrink-0" ref={menuRef}>
          <button onClick={(e) => { e.stopPropagation(); setOpen((p) => !p); }} className="p-1 rounded-md text-gray-300 dark:text-white/20 hover:text-gray-500 dark:hover:text-white/50 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors focus:outline-none">
          ...
          {open && (
            <div className="absolute right-0 top-6 z-50 w-44 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-white/10 shadow-lg overflow-hidden">
              <button onClick={(e) => { onMarkRead(e, item.id); setOpen(false); }} className="w-full flex items-center gap-2.5 px-3 py-2 text-xs text-gray-600 dark:text-white/60 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
